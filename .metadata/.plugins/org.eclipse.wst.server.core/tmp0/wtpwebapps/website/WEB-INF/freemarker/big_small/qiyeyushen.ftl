	...
	$(function(){
  	...
  	*/
  	function save(){
  		...
  		var data={memberId:memberId,blicUrl:blicUrl,lepUrl:lepUrl,occUrl:occUrl,blicTrcUrl:blicTrcUrl,company:company,contactName:contactName,
  		...
  		layer.load(2);
  		$.ajax({
			url:"${bootAppPath}/draftexchangecheck/save",
			type:"POST",
			data:data,
			dataType:"json",
	...
	/* function readFile(base64url,name,pathName){
		...
		$.ajax({
			...
			success:function(data){
				layer.closeAll();
				if(data.data.response == 'success'){
					$("#"+name).attr("src","${bootPic}"+data.data.data.base64Image);
