            ...
            <div class="pl20 pr20">
                ...
                <div class="flex-row flex-direction-column w tc lh30" id="isChuanWindow" style="display: none">
                    ...
                    <div class="w bte0e0e0 ble0e0e0 bre0e0e0 mt20">
                        ...
                        <!--列表展示-->
					    <div id="content1" class="oya h500 myScrollbar"></div>
                    </div>
                    <!--按钮操作-->
                    <div class="flex-row flex-justify-center lh30 mt40 mb20">
...
{{#each list}}
	<ul class="w bcwhite tc f14 lh40 ble0e0e0 bre0e0e0">
		<li class="bbe0e0e0 h40 oh">
			...
			<div class="fl bre0e0e0 w150">{{formatDate printtime}}</div>
			<div class="fl bre0e0e0 w150">{{formatDate endtime}}</div>
...
{{#each data}}
	<ul class="w oya oxh element" style="max-height: 200px">
		<li class="w bbe0e0e0 h30 oh tc c727272">
			<input type="number" name="paiForm{{id}}" id="allmoney{{id}}" class="w80 bre0e0e0 b0 f9 lh30 fl tc validate[custom[Okmoney]],validate[required],validate[custom[number]],validate[custom[money]]" value="{{toAllmoney allmoney}}" readonly="readonly">
			<input type="text" name="paiForm{{id}}" id="bank{{id}}" class="w100 bre0e0e0 b0 f9 lh30 fl tc oh wsn toe" value="{{toBank bank}}" title="{{bank}}" readonly="readonly">
			<input type="text" name="paiForm{{id}}" id="printtime{{id}}" class="w80 bre0e0e0 b0 f9 lh30 fl tc" value="{{formatDate printtime}}" readonly="readonly">
			<input type="text" name="paiForm{{id}}" id="endtime{{id}}" class="w80 bre0e0e0 b0 f9 lh30 fl tc" value="{{formatDate endtime}}" readonly="readonly">
			<input type="text" name="paiForm{{id}}" id="remarks{{id}}" class="w90 bre0e0e0 b0 f9 lh30 fl tc oh wsn toe" value="{{toRemarks remarks}}" title="{{remarks}}" readonly="readonly">
...
Handlebars.registerHelper('toIntent', function(intent, num,options) {
	...
	*/
	function search(){
		...
		var data = {minMoney:minMoney,maxMoney:maxMoney,state:"PASS"};
	...
	*/
		...
		setTimeout(function(){
			...
			if(data != null){
				...
				if(data.data.response == 'success'){
		            ...
		            layer.alert("您的库存清单已提交成功，请等待后台审核！");
    ...
    function saveForm(id) {
        if(!$("#allmoney"+id).validationEngine("validate")){
			$("#allmoney"+id).focus();
			return ;
		};
