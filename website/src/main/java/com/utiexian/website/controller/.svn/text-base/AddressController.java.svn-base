...
public class AddressController {
	...
	public String save(Address address,Model model){
		...
		try {
			...
			if(address.getState() == 0){
				addressService.updateDefaultByMemberId(address.getId(), address.getMemberId());
			}
			result.put("response", "success");
			result.put("msg", "操作成功");
		} catch (Exception e) {
	 ...
	 */
	@RequestMapping("get")
	public String get(String addressId,Model model){
		Map<String,Object> result = new HashMap<String, Object>();
		try {
			result.put("data", addressService.getById(Integer.valueOf(addressId)));
			result.put("response", "success");
			result.put("msg", "操作成功");
		} catch (Exception e) {
			result.put("response", "failed");
