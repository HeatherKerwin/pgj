...
public class BaofooController {
		...
		if(StringUtils.isNotBlank(trans_id)){//商户订单号
			...
			if(map!=null && map.get("resp_code")!=null && "0000".equals(map.get("resp_code").toString())){//支付成功
				...
				if(type!=null && type==0){//查询查复
					...
					model.addAttribute("redirect_url", "/inquiryreply/list");
					...
					model.addAttribute("redirect_url", "/discountorder/discount?ym=yp");
					...
					model.addAttribute("redirect_url", "/distributeorder/list");
				}
	...
	public String returnUrl(HttpServletRequest request)throws Exception{
		...
		data_content = RsaCodingUtil.decryptByPubCerFileBuffer(data_content,BaofooUtil.cerFlieName);
		...
		if(resp_code!=null && "0000".equals(resp_code.toString())){
			Object orderNo = ArrayData.get("refund_order_no");//退款商户订单号
			if(orderNo!=null){
				Discountrecord disc = discountrecordService.getByBnsNoDiscount(orderNo.toString());
				disc.setRefundState(1);//已退款
	 ...
	 * @author WKX
	 */
	@RequestMapping("/refund/inquiryreply")
	public String returnUrlInquiryreply(HttpServletRequest request)throws Exception{
		String data_content = request.getParameter("data_content");//回调参数
		if(data_content.isEmpty()){//判断参数是否为空
			return "返回数据为空";
		}
		...
		if(resp_code!=null && "0000".equals(resp_code.toString())){
			Object orderNo = ArrayData.get("refund_order_no");//退款商户订单号
			if(orderNo!=null){
				List<InquiryReply> list = inquiryReplyService.getByNo(orderNo.toString());
				if(list!=null && list.size()>0){
					InquiryReply ir = list.get(0);
					ir.setPayState(2);//已退款
					inquiryReplyService.updateInquiryReply(ir);
				}
			}
			return "OK";//处理完成在页面输出OK（必须）
		}
