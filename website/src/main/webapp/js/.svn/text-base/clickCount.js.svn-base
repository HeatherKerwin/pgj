	...
	$(document).ready(function (){
		...
		if(url.indexOf("360media")>-1){
			...
			from = 'ad7';
		}else if(url.indexOf("tg")>-1){
			from = 'baidu';
		}else if(url.indexOf("baidu")>-1){
			from = 'baidu';
		}else if(url.indexOf("sina")>-1){
			from = 'sina';
		}
	...
	function initClickCount(style){
		...
		$.ajax({
			type:"post",
			url:websitePath + '/tg/clickCount/initSave',//从后台获取ip
			data:{"url":url,"style":style,"uuid":uuid,"referrerUrl":referer},
			dataType:"text",
	...
	function countByIp(){
		$.ajax({
			type:"post",
			url:websitePath + "/tg/clickCount/countByIp",
			data:{},
			dataType:"text",
			success:function(data){
				if(data=="success"){
	...
	function save(style,code,href){
		...
		$.ajax({
			type:"post",
			url:websitePath + "/tg/clickCount/save",//从后台获取ip
...
//		var thisUrl = location.href;
		var url = location.href; //当前地址
//		var page_id = Math.random(); //页面唯一id
		var uuid = getUuid();
		if(setCookie("uuid", uuid) == ""){
			uuid = $.cookie("uuid");
		}
	...
	function count(style,code){
		$.ajax({
			type:"post",
			url:websitePath + "/tg/clickCount/count",
