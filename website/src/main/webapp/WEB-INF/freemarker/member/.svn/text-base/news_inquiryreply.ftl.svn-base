...
{{#each results}}
<tr data-id="{{id}}" order-id="{{discountrecordId}}" class="ha lh40 tc bbe0e0e0 {{readState}}">
    <td class="w40 bre0e0e0"><input type="checkbox" name="state" class="w13 h13 checkbox1 b0 mb13"></td>
...
function loadData(num){
	$('#pager').sjAjaxPager({
        url: "${basePath}/systeminfo/inquiryreply",
        pageIndex:1,
        pageSize:10,
        searchParam: {
        },beforeSend: function () {
	...
	$.ajax({
		...
		success:function(data){
			if(data.response=='success'){
				window.location.href = "${basePath}/inquiryreply/detail?no="+data.no;
			}
		}
	});
}
