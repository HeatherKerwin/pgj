            ...
            <div class="clearfix oh bcwhite mt20 pl3 pr3 pb6">
                <div class="fl clearfix oh w556 br5 bt3_e84c3d lh45 tc box-sizing box-shadow pb20">
                    ...
                    <div class="clearfix oh bcf9f9f9 c717583">
                        ...
                        <div class="fl w_33 appraisalFee none">打款金额</div>
                    </div>
                    <div class="clearfix oh pt10 pb10">
                ...
                <div class="fr clearfix oh w556 h420 br5 bt3_e84c3d box-sizing box-shadow pb20">
                    <div class="fl w_50 box-sizing pl30 lh24">
                        ...
                        <div class="w mt20 noVip none">
                            ...
                            <p class="ccccccc none noCoupon">无可用红包 <span class="f12 c3366cc"><a href="${basePath }/bisicmessage/vipMember">（成为会员享优惠）</a></span></p>
                    ...
                    <ul class="formGroup f14 c2d2d2d lh20">
                        ...
                        <li class="formItem">
                            <p class="ml120 ce84c3d">
                                只有关联指定银行对公账户方可收票。京东目前已经达成合作的指定银行有：平安银行<a href="${basePath}/PAB.pdf" style="text-decoration:underline;" download>(授权流程下载)</a>、中信银行<a href="${basePath}/ECITIC.pdf" style="text-decoration:underline;" download>(授权流程下载)</a>。</p>
                        </li>
                    </ul>
                    <div class="flex-row flex-align-self-center lh35 mt20">
...
<input type="hidden" id="cnapBankCode" name="cnapBankCode">
...
{{#each data}}
	<li class="fl w200 h86 bcf9f9f9 bae0e0e0 box-sizing pt6 pb6 pl10 pr10 br5 mt10 mr30 box-shadow {{toBank bindId}}" data-no="{{accountNo}}" data-b="{{bindId}}" data-name="{{bankName}}" onclick="selectBankClass(this);">
...
Handlebars.registerHelper('formatDate', function(num, options) {
	...
	function loadDate(){
		...
		if(data != null){
			if(data.data.response == 'success'){
				...
				if(data.order_type == "DISCOUNTRECORD"){
					...
					$(".orderType").html("银票订单");
				}else if(data.order_type == "DISCOUNTRECORDSP"){
					$(".orderType").html("商票订单");
					$(".orderyin1").addClass("none");
					$(".orderyin2").addClass("none");
					$("#deposit").html(200);
				...
				$(".singleRate").html(((data.singleRate)*100).toFixed(2));
    ...
    */
    ...
    $("#closeBtn ,.closeBtn").click(function(){
	    ...
	    $("#buyoutWindow").addClass("none");//一口价
	    
	    $("#bankWindow").addClass("none");//选卡
			...
			if(data.data.response == 'success'){
	            ...
	            if(data.data!=null){
	            	if(data.data.data.length==1){
		        		$("#selectJdCibNo").html(data.data.data[0].accountNo);
		        		$("#selectJdCibName").html(data.data.data[0].bankName);
		        		$("#dist_bind_id").val(data.data.data[0].bindId);
	...
	*/
	function confirmPrice(){
		...
		var url = '${bootAppPath}/dispatch/save/dist/price';
		var params = {id:did,order_type:order_type,memberId:memberId,orgId:orgId,orderId:orderId,jxts:jxts,
					txje:txje,txlx:txlx,deposit:deposit,todoor_price:todoor_price};
