...
<div class="flex flex-direction-column mt25 w1150 bc bcwhite pt30 pl25 pr25 pb25 mb20">
    ...
    <!--订单详情电票 -->
    <div class="flex-row flex-direction-column w bae0e0e0 mt10 pr">
    	...
    	<div class="flex-row flex-direction-row h50 lh50 bcf9f9f9 tc bbe0e0e0 c717583">
        	...
        	<div class="fl w158">票面金额</div>
        	<div class="fl w120">背书</div>
        	<div class="fl w200">承兑企业</div>
        	<div class="fl w129">保证金</div>
        	<div class="fl w320">票据特征</div>
    	</div>
    	<div class="flex-row flex-justify-space-between">
        	<div class="flex-row flex-direction-column Rright">
            	<div class="flex-row flex-direction-row tc lh45 pt10 pb10 bbe0e0e0">
                	...
                	<div class="w158 cd43c33 Rright"><span id="allmoney"></span></div>
                	<div class="w120 Rright"><span class="endorse"></span></div>
                	<div class="w200 Rright"><span id="bank"></span></div>
                	<div class="w129"><span id="deposit"></span></div>
            	</div>
				<div class="flex-row flex-justify-space-between w pt10 pb10">
					<div class="flex-row flex-direction-column w212 pl20 lh30 Rright">
						<div>开票日期：<span id="printtime"></span></div>
						<div>到期日期：<span id="endtime"></span></div>
						<div>计息天数：<span id="jxts"></span></div>
                	</div>
					<div class="flex-row flex-direction-row w599 flex-align-self-baseline mt10">
                    	<div class="w60 ml20">备注：</div>
                    	<p class="w500 mr20" ><span id="memberother"></span></p>
                	</div>
				</div>
        	</div>
        	<div class="flex-row flex-direction-row flex-wrap w320 lh34 tc">
            	<div class="c00a5f2 w96 h35 br10 ba00a5f2 ml35 mt16" id="type1"></div>
            	<div class="c00a5f2 w96 h35 br10 ba00a5f2 ml35 mt16 none" id="accept_time"></div>
				<div class="c00a5f2 w96 h35 br10 ba00a5f2 ml35 mt16 none" id="need_todoor">需要上门</div>
        	</div>
    	</div>
	</div>
    ...
    <div class="flex-row flex-direction-column w bae0e0e0 mt16 selectOrg">
        ...
        <!--资方信息-->
        <div class="flex-row flex-direction-column flex-justify-space-around bbe0e0e0 c717583 noOrg tc">
            ...
            <div class="pl20 pr20">
                ...
                <div class="flex-row flex-direction-column w lh30 none" id="evaluateWindow">
                   <div class="flex-row flex-direction-column bae0e0e0 br5 pl10 pr10 pt10 pb100 mt20">
                    ...
                    <!--按钮操作-->
                    <div class="flex-row flex-justify-center lh30 mt40 mb20">
                        <input type="button" id="comment" value="发表评价" class="w120 h35 ba bad43c33 bcd43c33 cwhite br3 mr20">
                    </div>
                </div>
            </div>
        </div>
...
<input type="hidden" class="allmoney"/>
...
{{#each listmap}}
	...
	<label class="{{toCompany todoor_price 2}} choseJigou" for="choseJG{{distId}}">
		<div class="flex-row flex-direction-row h50 lh50 bcf9f9f9 f13 tc bbe0e0e0 c717583 pr">
			<img  src="https://img.utiexian.com/website/180903jingdong/order/xingye-small.png" class="chooseBankIcon {{toStatus dist_v_acct_acct_no dist_bind_id 1}}">
			<img  src="https://img.utiexian.com/website/180903jingdong/order/jingdong-small.png" class="chooseBankIcon {{toStatus dist_v_acct_acct_no dist_bind_id 2}}">
...
Handlebars.registerHelper('toCompany', function(todoor_price,num, options) {
	...
	*/
	function exhibitionOrder(){
		...
		var params = {discId:id,order_type:'DISCOUNTRECORDSP'};
		var data = bootPost(url,params);
		console.log(data);
		if(data != null){
			if(data.data.response == 'success'){
				...
				if(data.orderflag == 0){
					...
					var date1 = new Date();
					var date2 = new Date(data.create_time); 
				...
				$("#allmoney").html(data.allmoney+"万元");
				$(".allmoney").val(data.allmoney);
				$("#endtime").html(data.endtime);
				$("#bank").html(data.bank);
				$(".endorse").html(data.endorse+"手");
				$("#jxts").html(data.jxts+"天");
				...
				if(data.type1 == 1){
					...
					$("#type1").html("电票");
					$("#accept_time").removeClass("none");
					if(data.accept_time == 1){
						$("#accept_time").html("一年期");
					}else{
						$("#accept_time").html("半年期");
					}
				}
	...
	*/
	function selectUpdate(){
		...
		if(type1 == 1){
			...
			if(jd_status){//京东合同
				$("#iframe").attr("src","${basePath}/jd/agreements/contract?distId="+distId+"&orderType=DISCOUNTRECORDSP");
			}else{//兴业合同
				$("#iframe").attr("src","${basePath}/agreements/contract?distId="+distId+"&orderType=DISCOUNTRECORDSP");
			}		
	...
	*/
	function agreeContract(){
		...
		var url = '${bootAppPath}/discountrecordsp/update/select';
    ...
    */
	...
	$("#evaluateBtn").click(function(){
		...
		if(type1 == 1){
			$("#comment_type").html("商纸");
		}else{
			$("#comment_type").html("商电");
		}
	    $("#maskWindow").removeClass("none");
	...
	*/
	$("#endorse").click(function(){
		...
		var url = '${bootAppPath}/discountrecordsp/update/endorsetime';
	...
	*/
	$("#gathering").click(function(){
		...
		var url = '${bootAppPath}/discountrecordsp/update/confirmaccounttime';
	...
	*/
	$("#comment").click(function(){
		...
		var params = {service:service,speed:speed,price:price,dcrdId:dcrdId,dtboId:dtboId,role:0,type:1,operatorId:memberId,content:content};
		var data = bootPost(url,params);
	...
	*/
	$(".download").click(function(){
		...
		if(dist_bind_id!=null){
			...
			var params = {orderId:discId,orderType:"DISCOUNTRECORDSP"};
			var data = bootPost(url,params);
			console.log(data);
			if(data != null){
				if(data.data.response == 'success'){
