	        ...
	        <div class="orderDetailCon">
	            ...
	            <div class="detailHead">
	                ...
	                <div class="ticketType">
	                    <img src="https://img.utiexian.com/website/180606/yindian.png" alt="银票电票"  class="{{toType1State type1 1}}">
						<img src="https://img.utiexian.com/website/180606/yinzhi.png" alt="银票纸票" class="{{toType1State type1 2}}">
	                </div>
	                <!--下单信息-->
...
Handlebars.registerHelper('toOperation', function(orderflag, deposit_state ,type1,c_id,sum,num, options) {
	...
	* 页面初始化 加载数据
	...
	function loadDate(){
		...
		if(org == 0){//查询所有的订单
			data = {memberId:memberId,orderType:'DISCOUNTRECORD'};
        }else if(org == 1){//查询待支付的订单
        	data = {memberId:memberId,orderType:'DISCOUNTRECORD',depositState:0,orderflags:1};
        }else if(org == 2){//查询交易中的订单
        	data = {memberId:memberId,orderType:'DISCOUNTRECORD',depositState:1,orderflags:'1,2,4,5,7'};
        }else if(org == 3){//查询待评价的订单
        	data = {memberId:memberId,orderType:'DISCOUNTRECORD',orderflags:3,comment:1};
        }else if(org == 4){//查询完成的订单
        	data = {memberId:memberId,orderType:'DISCOUNTRECORD',orderflags:3,comment:0};
        }else if(org == 5 ){//查询无效的订单
        	data = {memberId:memberId,orderType:'DISCOUNTRECORD',orderflags:0};
        }
	...
	*/
	function details(){
		...
		var map = new Map();
		map.put("_PAGE", "/discountorder/discountorder");//必传
