...
<a id="ddd" target='_blank' class="none"  href="javascript:alert('1')"></a>
<!--支付弹窗-->
...
Handlebars.registerHelper('toButton', function(orderflag,c_id,cib_cancel,num,options) {
	...
	*/
	function loadDate(){
		...
		if(org == 0){
			data = {orgId:orgId,orderType:'DISTRIBUTEORDER'};
		}else if(org == 1){
			data = {orgId:orgId,orderType:'DISTRIBUTEORDER',orderflags:'1,2,4,5,7'};
		}else if(org == 2){
			data = {orgId:orgId,orderType:'DISTRIBUTEORDER',orderflags:3,comment:1};
		}else if(org == 3){
			data = {orgId:orgId,orderType:'DISTRIBUTEORDER',orderflags:3,comment:0};
		}else if(org == 4){
			data = {orgId:orgId,orderType:'DISTRIBUTEORDER',orderflags:0};
		}
	...
	*/
	function details(){
		...
		var map = new Map();
		map.put("_PAGE", "/hall/receiveOrder");
		map.put("id", id);
		_OPENPAGE_FORM(map);
	};
//日历
