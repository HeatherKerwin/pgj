	...
	canvasAll : function(){
		...
		var ev = {
			canvasInit : function(){
				...
				{
					var x = i * space;
					dataAna.canvasAll().canvasDraw.drawLine(x,0,x,$height,"#ffffff");
				}

				for (var i = 1,j=lenY; i <= j; i++) {
					var y = i * space;
					dataAna.canvasAll().canvasDraw.drawLine(0,$height-y,$width,$height-y,"#ffffff");
				};
			...
			canvasDraw  : {
				...
				drawText : function(text,x,y,font,align,base,color){//此处可修改横纵坐标大小字体颜色
					if(!font || font=="")
					font = "12px tohoma";
	...
	dataHandling : function(){
		...
		var ev = {
			nameHanding : function(name,title,style,pos){
				...
				dataAna.canvasAll().canvasDraw.drawText(title,$width/2,6,"18px Microsoft YaHei","center","top","#d43c33");
			},
