...
public class Pkipair {
	...
	public String signMsg( String signMsg) {
		...
		try {
			...
			URL in = this.getClass().getResource("99bill-rsa.pfx"); 
			JarURLConnection jarURLConnection = (JarURLConnection)in.openConnection();
			BufferedInputStream ksbufin = new BufferedInputStream(jarURLConnection.getInputStream());
			
			
			
			

			char[] keyPwd = "50965066".toCharArray();
