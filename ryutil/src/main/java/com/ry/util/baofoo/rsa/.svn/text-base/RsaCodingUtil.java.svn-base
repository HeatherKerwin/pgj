...
import com.ry.util.baofoo.util.FormatUtil;
...
//			//log.error("解密内容异常", e);
	...
	/**
	 * 根据私钥文件加密（自定义读取Buffer）
	 * @param src
	 * @param pfxFlieName
	 * @param priKeyPass
	 */
	public static String encryptByPriPfxFileBuffer(String src, String pfxFlieName, String priKeyPass) {
		PrivateKey privateKey = RsaReadUtil.getPrivateKeyFromBufferFile(pfxFlieName, priKeyPass);
		if (privateKey == null) {
			return null;
		}
		return encryptByPrivateKey(src, privateKey);
	...
	/**
	 * 指定Cer公钥路径解密（自定义读取Buffer文件）
	 * @param src
	 * @param pubCerFlieName
	 */
	public static String decryptByPubCerFileBuffer(String src, String pubCerFlieName) {
		PublicKey publicKey = RsaReadUtil.getPublicKeyFromBufferFile(pubCerFlieName);
		if (publicKey == null) {
			return null;
		}
		return decryptByPublicKey(src, publicKey);
