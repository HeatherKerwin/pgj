...
import { DayPageB } from "../../manage/org/books/day";
import { WeekPageB } from "../../manage/org/books/week";
import { MonthPageB } from "../../manage/org/books/month";
import { YearPageB } from "../../manage/org/books/year";
import { RecordPageB } from "../../manage/org/books/record";
import { ManageeditPageB } from "../../manage/org/books/edit";
import { RemindPageB } from "../../manage/org/books/remind";
...
export class ManagePageB {
  ...
  // 账簿
  public content:any;
  public notifications:number;//提醒图标
  // 年月日
  ...
  constructor(
    ...
    this.storage.get('ORGID').then((data)=>{
      this.orgId=data;
      let url=this.apiSev.getWapUrl()+'app/pieChart?belong=1&type='+this.Statistics.type+'&memberId='+this.orgId;
...
/*    this.getTime();*/
  ...
  changeStart(){
    if (this.search.limit=='0'){
      this.search.end = new Date(new Date(this.search.start).getTime() + 180 * 24 * 60 * 60 * 1000).toISOString();
      this.search.min = new Date(this.search.start).toISOString();
    }else if(this.search.limit=='1') {
      this.search.end = new Date(new Date(this.search.start).getTime() + 360 * 24 * 60 * 60 * 1000).toISOString();
      this.search.min = new Date(this.search.start).toISOString();
    };
  }
...
//--------------账簿开始-----------------------
  ...
  booksData() {
    this.storage.get('ORGID').then((data)=>{
      this.apiSev.api("get", "zhangbu/index/?memberid="+data+"&belong=1", (res) => {
        this.content = res;
  ...
  //跳转日周月年
  dayPage() {
    this.navCtrl.push(DayPageB);
  }
  weekPage() {
    this.navCtrl.push(WeekPageB);
  }
  monthPage() {
    this.navCtrl.push(MonthPageB);
  }
  yearPage() {
    this.navCtrl.push(YearPageB);
  }
  recordPage() {
    this.navCtrl.push(RecordPageB);
  }
//--------------账簿结束-----------------------
...
//--------------已贴现未贴现搜索开始-----------------------
   ...
   */
  ToExportPage(){
    this.navCtrl.push(ExportPage,{ROLE:1});
  }
  //选择承兑期限
  searchLimit(){
  ...
  // 搜索
  searchBtn(type){
    let starttime = new Date(this.search.start).getFullYear()+"-"+new Date(this.search.start).getMonth()+"-"+new Date(this.search.start).getDate();
    let endtime = new Date(this.search.end).getFullYear()+"-"+new Date(this.search.end).getMonth()+"-"+new Date(this.search.end).getDate();
...
//--------------已贴现未贴现列表开始-----------------------
  discount(isTX) {
    this.storage.get('ORGID').then((data)=>{
      this.apiSev.api("get", "zhangbu/piaojulist/?memberId="+data+"&istiexian="+isTX+"&belong=1", (res) => {
        if (isTX==1){
          this.value1s.length=0;
          this.value1s = res;
        }else if (isTX==0){
...
//-------------- 统计分析开始 -----------------------
  ...
  typeChange() {
    ...
    if (this.Statistics.type == 4) {
      ...
      url = this.apiSev.getWapUrl() + 'app/pieChart?belong=1&type=' + this.Statistics.type + '&memberId=' + this.orgId + '&start=' + this.Statistics.diyStartDate + '&end=' + this.Statistics.diyEndDate;
    }
    console.log(url);
    this.browser.url = url;
    this.browser.secUrl = this.sanitizer.bypassSecurityTrustResourceUrl(url);
  ...
  /*自定义结束日期最小值*/
  minChange(){
    this.Statistics.minDate =new Date(this.Statistics.diyStartDate).toISOString();
  }
//-------------- 统计分析结束 -----------------------
