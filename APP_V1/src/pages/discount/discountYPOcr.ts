...
@Component({
  selector: 'page-discountYPOcr',
  templateUrl: 'discountYPOcr.html'
})


export class DiscountYPOcrPage {
    ...
    this.camera.getPicture(options).then((imageData) => {
      ...
      this.apiSev.api("newpost", "upload/image", (res) => {
        ...
        if (data.response == 'success') {
          ...
          if(data.data.ocrInfo.type2Map!=null&&data.data.ocrInfo.type2Map!=''&&data.data.ocrInfo.type2Map!='undefined'){if(data.data.ocrInfo.type2Map.type2!=null&&data.data.ocrInfo.type2Map.type2!=''&&data.data.ocrInfo.type2Map.type2!='undefined'){this.discount.type2=data.data.ocrInfo.type2Map.type2;}}     //结束日期


              if (this.bankNameAccuracy==1&&this.bankNoAccuracy==1&&this.noAccuracy==1&&this.moneyAccuracy==1&&this.endDateAccuracy==1&&this.discount.bankName!='undefined'&&this.discount.bankNo!='undefined'&&this.discount.no!='undefined'&&this.discount.money!='undefined'&&this.discount.endDate!='undefined'&&this.discount.bankName != null && this.discount.bankName != '' && this.discount.bankNo != null && this.discount.bankNo != '' && this.discount.no != null && this.discount.no != '' && this.discount.money != null && this.discount.money != '' &&  this.discount.endDate != null && this.discount.endDate != '') {
                this.navCtrl.push(DiscountYPPage, {
                  //TEMP_START: this.discount.beginDate,
                  TEMP_END: this.discount.endDate,
                  TEMP_ALLMONEY: this.discount.money,
