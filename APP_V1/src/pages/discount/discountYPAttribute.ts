...
import { DiscountYPOcrPage } from "./discountYPOcr";
...
@Component({
  selector: 'page-discountYPAttribute',
  templateUrl: 'discountYPAttribute.html'
})


export class DiscountYPAttributePage {
      ...
      if (data.response == 'success') {
        if(data.data.cib!=null&&data.data.cib!=''&&data.data.cib!='undefined'){
          ...
          if(data.data.cib.status==2&&data.data.cib.cib_check_state=='PASS'){
            this.navCtrl.push(DiscountYPOcrPage,{TEMP_TYPE1:type1,ORDERINVENTORY:this.orderinventory});
          }else {
            //兴业绑定开户判断
            this.maskDiv = true;
    ...
    this.apiSev.api("newpost", "jdjr/cib/account", (res) => {
      ...
      if (data.response == 'success') {
        if(data.data.cib!=null&&data.data.cib!=''&&data.data.cib!='undefined'){
          //是否兴业开户
          if(data.data.cib.status==2&&data.data.cib.cib_check_state=='PASS'){
            this.navCtrl.push(DiscountYPPage,{TEMP_TYPE1:type1,ORDERINVENTORY:this.orderinventory});
          }else {
            //兴业绑定开户判断
            this.maskDiv = true;
            this.Authentication = true;
