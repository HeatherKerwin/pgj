 ...
 * @author WKX
 * 备注：（以下是两个支付方式）
 * https://test.utiexian.com/rywap/account/pay?money=10&memberId=91&type=BILL99
 * https://test.utiexian.com/rywap/account/pay?money=10&memberId=91&type=BAOFOO
 */
@Controller
...
public class AccountController {
	...
	public String baofoo(String orderNo,Model model){
		try {
			...
			model.addAttribute("user_id", log.getMemberId());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "pay/baofoo/pay";
	}
	...
	public String baofoo6(String business_no,String sms_code,String trans_id,Model model) throws Exception{
		...
		if(StringUtils.isNotBlank(trans_id)){//商户订单号
			...
			if(map!=null && map.get("resp_code")!=null && "0000".equals(map.get("resp_code").toString())){//支付成功
				...
				model.addAttribute("redirect_url", "/account/baofoo/pay/cb/page?orderNo="+map.get("trans_no")+"&orderAmount="+map.get("succ_amt"));
				model.addAttribute("code", "T");//成功
				return "pay/baofoo/result";
			}else{
	 ...
	 */
	@RequestMapping("/baofoo/pay/cb/page")
