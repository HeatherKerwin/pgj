...
<body>
    ...
    <div class="nav">
        ...
        <a style="margin: 0px 5%;float:right;" href="${basePath}/account/baofoo/pay?orderNo=${orderNo}">去支付></a>
    </div>
...
function bindCard(){
	...
	$.ajax({
		type : "POST",
		url : "${basePath}/account/baofoo/baofoo2",
		datatype : "json",
		data : params,
		success : function(data) {
			var res = jQuery.parseJSON(data);
...
$("#prompt-know").on('click', function(){
	if("T"==_state){
		window.location.href = "${basePath}/account/baofoo/pay?orderNo=${orderNo}";
	}
	$("#maskWindow").addClass("none");
