...
<body>
    ...
    <div class="nav">
        ...
        <div class="w form form-pay">
            <ul class="list">
                <li id="toggle-pay">
                    ...
                    <div class="fl pay-money">${money}元</div>
            ...
            <ul class="list-pay none" id="list-pay">
                ...
                <li>
                    <div class="fl">支付编号：</div>
                    <div class="fl">${orderNo}</div>
                </li>
                <li>
                    <div class="fl">创建时间：</div>
                    <div class="fl">${orderDate}</div>
                </li>
            </ul>
        </div>
        ...
        <div class="w form">
            ...
            <a class="bank-add" href="${basePath}/account/baofoo/bindcard?orderNo=${orderNo}">
    ...
    function time(o) {
        ...
        if (wait == 60) {
            ...
            var htmlobj = $.ajax({ type: "POST",
                url: "${basePath}/account/baofoo/baofoo5",
                datatype: "xml",
                data: {
...
function initZhiwen(){
	...
	var t1 = window.setInterval(function(){
		$.ajax({ type: "POST",
	        url: "${basePath}/account/baofoo/zhiwen",
	        datatype: "json",
	        data: {
 ...
 */
 $(".pay-delete").on("click",function () {
	if(confirm("解除绑定该卡？")){
		...
		$.ajax({
			type: "POST",
	        url: "${basePath}/account/baofoo/baofoo3",
	        datatype: "json",
	        data: {
	        	type: "3",
