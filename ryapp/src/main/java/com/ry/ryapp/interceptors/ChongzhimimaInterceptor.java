...
@Controller
public class ChongzhimimaInterceptor implements HandlerInterceptor {
	@Resource
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
		...
		if (StringUtils.isNoneBlank(member_id)&&!"undefined".equals(member_id)) {
			...
			// 企业
			if("0".equals(belong)){
				actionlog.setCode(Constant.MIMACHONGZHI);
			}else{
				actionlog.setCode(Constant.MIMACHONGZHI_ORG);
			}
			actionlog.setMemberId(Integer.valueOf(member_id));
			/*else if (org != null && 1 == org.getType()) {
				actionlog.setCode(Constant.MIMACHONGZHI_ORG); // 这是ORG企业的常量
