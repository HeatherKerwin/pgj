...
public class JiGouShangpiaoQuXiaoDingDanInterceptor implements HandlerInterceptor {
	@Resource
	...
	public void afterCompletion(HttpServletRequest request,
			...
			String jujue = request.getParameter("jujue");
			Actionlog actionlog = new Actionlog();		
			if(StringUtils.isNoneBlank(member_id)&&!"undefined".equals(member_id)){
				if(jujue!=null && jujue.trim()!=""){
					actionlog.setCode(Constant.SHANGPIAOJUJUEDINGDAN_ORG); //机构 拒绝订单
				}else{
					actionlog.setCode(Constant.SHANGPIAOQUXIAODINGDAN_ORG); //机构 取消订单
				}
