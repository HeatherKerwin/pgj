...
@Controller
public class ShichangxinxiInterceptor implements HandlerInterceptor{
	@Resource
	...
	public void afterCompletion(HttpServletRequest request,
		...
		if (!"180.166.201.178".equals(ipadr)) {
			...
			actionlog.setCode(Constant.SHICHANGXINXI);
		...
		String ipadr = HttpUtil.getIpAddr(request);
			...
			if(StringUtils.isNoneBlank(member_id)&&!"undefined".equals(member_id)){
				if ("0".equals(belong)) {//企业
					actionlog.setCode(Constant.SHICHANGXINXI_LIST);	//这是BNS企业的常量
					actionlog.setMemberId(Integer.valueOf(member_id));
				}else  {
					actionlog.setCode(Constant.SHICHANGXINXI_LIST_ORG);	//这是ORG企业的常量
					actionlog.setMemberId(Integer.valueOf(member_id));
				}
				String from = request.getParameter("from");
				if (from != null) {
