...
@Controller
public class DingdanzhongxinInterceptor implements HandlerInterceptor{
	@Resource
	...
	public void afterCompletion(HttpServletRequest request,
			...
			if(StringUtils.isNoneBlank(member_id)&&!"undefined".equals(member_id)){
				//这是BNS企业的常量
				actionlog.setCode(Constant.DINGDANZHIONGXIN);
				actionlog.setMemberId(Integer.valueOf(member_id));
			}else if(StringUtils.isNoneBlank(orgid)){
				//这是ORG机构的常量
				actionlog.setCode(Constant.DINGDANZHIONGXIN_ORG);
				actionlog.setMemberId(Integer.valueOf(orgid));
			}
