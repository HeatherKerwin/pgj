...
@Controller
public class WanshanxinxiInterceptor implements HandlerInterceptor{
	@Resource
			...
			String ipadr = HttpUtil.getIpAddr(request);
				...
				if(StringUtils.isNoneBlank(member_id)&&!"undefined".equals(member_id)){
					if ("0".equals(belong)) {//企业
						actionlog.setCode(Constant.WANSHAN);	//这是BNS企业的常量
						actionlog.setMemberId(Integer.valueOf(member_id));
					}else {
						actionlog.setCode(Constant.WANSHAN_ORG);	//这是ORG企业的常量
						actionlog.setMemberId(Integer.valueOf(member_id));
					}
