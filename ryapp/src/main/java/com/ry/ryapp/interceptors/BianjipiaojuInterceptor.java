...
@Controller
public class BianjipiaojuInterceptor implements HandlerInterceptor {
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
		...
		if (StringUtils.isNoneBlank(member_id)&&!"undefined".equals(member_id)) {
			if ("0".equals(belong)) {// 企业
				actionlog.setCode(Constant.BIANJIPIAOJU); // 这是BNS企业的常量
				actionlog.setMemberId(Integer.valueOf(member_id));
			} else {
				actionlog.setCode(Constant.BIANJIPIAOJU_ORG); // 这是ORG企业的常量
				actionlog.setMemberId(Integer.valueOf(member_id));
			}
