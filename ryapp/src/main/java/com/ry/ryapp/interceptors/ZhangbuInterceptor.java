 ...
 */
@Controller
public class ZhangbuInterceptor implements HandlerInterceptor {
...
//			actionlog.setCode(Constant.ZHANGBU);
//			actionlogService.addActionlog(actionlog);
			...
			if (StringUtils.isNotBlank(member_id)&&!"undefined".equals(member_id)) {
				if ("0".equals(belong)) {
					...
					Org org = orgService.getById(Integer.valueOf(member_id));
					actionlog.setCode(Constant.ZHANGBU_ORG);
