...
@Controller
public class OrgSavePayVoucherInterceptor implements HandlerInterceptor{
	@Resource
	...
	public void afterCompletion(HttpServletRequest request,
			...
			actionlog.setCode(Constant.SAVEPAYVOUCGER);
			if(StringUtils.isNotBlank(org_id)){
				actionlog.setMemberId(Integer.parseInt(org_id));
			}else{
				String memberId = request.getParameter("memberId");
				if(StringUtils.isNotBlank(memberId)&&!"undefined".equals(memberId))actionlog.setMemberId(Integer.valueOf(memberId));
			}
