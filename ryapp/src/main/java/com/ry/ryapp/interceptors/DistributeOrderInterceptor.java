...
@Controller
public class DistributeOrderInterceptor implements HandlerInterceptor{
	@Resource
	...
	//执行Handler完成执行此方法
	//应用场景：统一异常处理，统一日志处理		联系承兑方
	@Override
	public void afterCompletion(HttpServletRequest request,
			HttpServletResponse response, Object handler, Exception ex) throws Exception {
			String ipadr = HttpUtil.getIpAddr(request);
			String dtbo_id = request.getParameter("dtboId");
			if (StringUtils.isNotBlank(dtbo_id)) {
				DistributeOrder order = distributeOrderService.getById(Integer.valueOf(dtbo_id));
				Org org = orgService.getById(order.getOrgId());
				Actionlog actionlog = new Actionlog();		
				actionlog.setCode(Constant.LIANXICHENGDUI);			//TODO:这是BNS企业的常量
