...
@Controller
public class XuanzejiaoseInterceptor implements HandlerInterceptor{
	@Resource
	...
	public void afterCompletion(HttpServletRequest request,
			...
			if(StringUtils.isNoneBlank(member_id)&&!"undefined".equals(member_id)){
				if ("0".equals(type)) {//企业
					actionlog.setCode(Constant.XUANZEJIAOSE);	//这是BNS企业的常量
					actionlog.setMemberId(Integer.valueOf(member_id));
				}else {
					actionlog.setCode(Constant.XUANZEJIAOSE_ORG);	//这是ORG企业的常量
					actionlog.setMemberId(Integer.valueOf(member_id));
				}
