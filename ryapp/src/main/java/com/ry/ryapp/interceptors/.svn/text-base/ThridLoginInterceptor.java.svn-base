...
public class ThridLoginInterceptor implements HandlerInterceptor {
	@Resource
	...
	public void afterCompletion(HttpServletRequest request,
			...
			if (StringUtils.isBlank(member_id)&&!"undefined".equals(member_id)) {
				Member m = (Member) request.getSession().getAttribute("member");
				if (m != null && m.getId() != null)
					member_id = m.getId().toString();
			}
			String type = request.getParameter("type");
			Actionlog actionlog = new Actionlog();
			if(StringUtils.isNotBlank(member_id) && StringUtils.isNotBlank(type)){
				if("1".equals(type)){
					actionlog.setCode(Constant.WECHARTLOGIN);
				}else if("2".equals(type)){
					actionlog.setCode(Constant.QQLOGIN);
				}else if("3".equals(type)){
					actionlog.setCode(Constant.WEBOLOGIN);
				}
