...
@Controller
public class ChaxunchafuInterceptor implements HandlerInterceptor{
	@Resource
		...
		public void afterCompletion(HttpServletRequest request,
				...
				String ipadr = HttpUtil.getIpAddr(request);
					...
					if(StringUtils.isNoneBlank(member_id)&&!"undefined".equals(member_id)){
						if ("0".equals(orgType)) {//企业
							actionlog.setCode(Constant.CHAXUNCHAFU);	//这是BNS企业的常量
							actionlog.setMemberId(Integer.valueOf(member_id));
						}else  {
							actionlog.setCode(Constant.CHAXUNCHAFU_ORG);	//这是ORG企业的常量
							actionlog.setMemberId(Integer.valueOf(member_id));
						}
