...
public class HttpClient {
	 ...
	 * @param url 目标URL
	 * @param appid 平台appid
	 * @param exchPrivateKey 平台加签私钥
	 * @param ufmPublicKey 兴资管验签公钥
	 * @param requestMsg 请求消息
	 * @param method HTTP方法
	 * @return 返回json
	 * @throws Exception
	 */
	public String send(String url, String appid, String exchPrivateKey, String ufmPublicKey, String requestMsg, METHOD method) throws Exception {
		//1.BASE64编码
		...
		case DELETE:
			...
			break;
		case GET:
			responseTxt = get(url, encoded,signature);
			break;
		default:
			break;
		}
	...
	public String get(String url,String message,String signature) throws Exception {
		CloseableHttpClient client = getHttpClient();
		CloseableHttpResponse response = null;
		try {
			url += "?message="+URLEncoder.encode(message, "UTF-8")+"&signature="+URLEncoder.encode(signature, "UTF-8");
     ...
     * 文件上传
     * @param url 服务地址
     * @param fileUrl 要上传的文件
     * @param appid 平台id
     * @param exchPrivateKey 平台私钥
     * @param ufmPublicKey 兴存管公钥
     * @param requestMsg 请求消息
     */
    public String upload(String url, String fileUrl, String appid, String exchPrivateKey, String ufmPublicKey, String requestMsg)throws Exception {
        String encoded = Base64.encodeBase64String(requestMsg.getBytes("UTF-8"));//1.base64编码
        
        String forSign = StringUtils.join(appid, encoded);//2.左边加上appid后生成待签字符串
        String signature = RSASignature.sign(forSign, exchPrivateKey, "UTF-8");
        ...
        try {
            ...
            mEntityBuilder.addBinaryBody("file", FileUtil.createFileByUrl(fileUrl, "jpg"));
