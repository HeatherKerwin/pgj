...
<body>
    <div id="contet">    	
    	<form id="form1" name="form1" method="post" action="<%=path %>/pic/save/?code=index_ad">
		    	...
		    	<ol>
		            ...
		            <c:choose>
		            	<c:when test="${appimageList[0].id >0 }">
		            		<img class="img" id="img1" src="${appimageList[0].path}" alt="图片在此显示" onerror="this.src='<%=path%>/images/nopic.png;'"/>
		            		<input type="hidden" id="picpath1" name="picpath1" value="${appimageList[0].path}"/>
		            	</c:when>
		            	<c:otherwise>
		            		<img class="img" id="img1" alt="图片在此显示" onerror="this.src='<%=path%>/images/nopic.png;'"/>
