...
<div id="top">
	<div class="left">
		<button type="button" class="btn btn-success" id="add" onclick="javascript:window.location.href='<%=path %>/admin/badd/'">+ 新增当日价格</button>
		&nbsp;&nbsp;&nbsp;
...
<table class="table table-bordered table-hover">
	...
	<tbody>
	<c:forEach items="${daypriceList}" var="dayprice">
		<tr>
			<td class="text-center td-checkbox"><input type="checkbox" id="" value="${dayprice.id }" name="idcheckbox" /></td>
			<td class="text-left td-name">
				<c:if test="${dayprice.type1==1 }">国股</c:if>
				<c:if test="${dayprice.type1==2 }">城商</c:if>
				<c:if test="${dayprice.type1==3 }">外资</c:if>
				<c:if test="${dayprice.type1==4 }">农商</c:if>
				<c:if test="${dayprice.type1==5 }">农合</c:if>
				<c:if test="${dayprice.type1==6 }">农信</c:if>
				<c:if test="${dayprice.type1==7 }">村镇</c:if>
			</td>
			<td class="text-left td-name">
				<c:if test="${dayprice.type2==1 }">大票</c:if>
				<c:if test="${dayprice.type2==2 }">小票</c:if>
			</td>
			<td class="text-left td-name">
			...
			<td class="text-left td-name">
				<c:if test="${dayprice.type2==1 }"><input type="text" value="${dayprice.price }" id="price${dayprice.id}"/>‰</c:if>
				<c:if test="${dayprice.type2==2 }"><input type="text" value="${dayprice.price }" id="price${dayprice.id}"/></c:if>
			</td>
			<td class="text-left td-name">
				<a href="javascript:void(0)" onclick="update(${dayprice.id})">修改</a>
			</td>
		</tr>
	</c:forEach>
	</tbody>
...
<script language="javascript">
	...
	function jumpPage(jumpPageNumber){
		window.location.href="<%=path%>/admin/daypricesearch/?currentPage="+jumpPageNumber;
	}
	...
	function search(){
		...
		var type4 = $("#type4").val();
		var day = $("#day").val();
		window.location.href="<%=path%>/admin/daypricesearch/?type1="+type1+"&type4="+type4+"&day="+day;
	}
	
	function update(id){
		window.location.href="<%=path%>/admin/updateprice/?id="+id+"&price="+$('#price'+id).val()+"&currentPage=${pageModel.currentPage}";
	}
</script>
</body>
</html>