...
<body>
<form method="post" action="<%=path %>/piaojujiangtang/update/" onsubmit="return checkForm()" id="form1">
<div id="top">
	<div class="left">
...
<div id="edit">
	...
	<div id="edit-main">
		<div class="edit-box">
			<div class="label"><label>类型</label><input type="hidden" value="${piaojujiangtang.id }" name="id"/></div>
			<div class="">
				<select name="type">
					<option value="1" <c:if test="${piaojujiangtang.type==1 }">selected="selected"</c:if>>新手指引</option>
					<option value="2" <c:if test="${piaojujiangtang.type==2 }">selected="selected"</c:if>>票据知识</option>
					<option value="3" <c:if test="${piaojujiangtang.type==3 }">selected="selected"</c:if>>如何验票</option>
					<option value="4" <c:if test="${piaojujiangtang.type==4 }">selected="selected"</c:if>>法律法规</option>
				</select>
			</div>
		</div>
		<div class="edit-box">
		...
		<div class="edit-box">
			...
			<div class="">
				<input name="publishtime" id="day" type="text" onclick="SelectDate(this,'yyyy-MM-dd hh:mm')" readonly style="width:175px;cursor:pointer" value="<fmt:formatDate value='${piaojujiangtang.publishtime}' pattern='yyyy-MM-dd HH:mm' />"/>
			</div>
		</div>
		<br/>
	</div>
