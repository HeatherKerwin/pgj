...
<body>
<form method="post" action="<%=path %>/historyprice/updateprice/" onsubmit="return checkForm()" id="form1">
<div id="edit">
	<table width="800px">
		<tr>
			<td colspan="4" align="center" width="400px">日期<input type="text" name="day" id="day" value="${day }" placeholder="日期" onclick="SelectDate(this,'yyyy-MM-dd')" readonly onchange="search()"/></td>
			
			<td colspan="4" align="center" width="400px">区域<select name="type4" id="type4" onchange="search()"><option value="1" <c:if test="${type4==1 }">selected="selected"</c:if>>长三角</option><option value="2" <c:if test="${type4==2 }">selected="selected"</c:if>>珠三角</option><option value="3" <c:if test="${type4==3 }">selected="selected"</c:if>>华中</option><option value="4" <c:if test="${type4==4 }">selected="selected"</c:if>>环渤海</option><option value="5" <c:if test="${type4==5 }">selected="selected"</c:if>>西南</option></select></td>
		</tr>
		<tr>
			<td width="20px">&nbsp;</td>
			<td width="100px">&nbsp;</td>
			<td width="140px" align="center">月利息</td>
			<td width="140px" align="center">涨跌</td>
			<td width="20px">&nbsp;</td>
			<td width="100px">&nbsp;</td>
			<td width="140px" align="center">月利息</td>
			<td width="140px" align="center">涨跌</td>
...
<script>
	function search(){
		location.href="<%=path%>/historyprice/search/?day="+$("#day").val()+"&type4="+$("#type4").val();
	}
</script>
</body>
</html>