...
<body>
<form method="post" action="<%=path %>/news/update/" onsubmit="return checkForm()" id="form1">
<div id="top">
	<div class="left">
...
<div id="edit">
	...
	<div id="edit-main">
		<div class="edit-box">
			<div class="label"><label>类型</label><input type="hidden" value="${news.id }" name="id"/></div>
			<div class="">
				<select name="type">
					<option value="1" <c:if test="${news.type==1 }">selected="selected"</c:if>>票据新闻</option>
					<option value="2" <c:if test="${news.type==2 }">selected="selected"</c:if>>银行动态</option>
					<option value="3" <c:if test="${news.type==3 }">selected="selected"</c:if>>市场分析</option>
					<option value="4" <c:if test="${news.type==4 }">selected="selected"</c:if>>相关新闻</option>
				</select>
			</div>
		</div>
		<div class="edit-box">
		...
		<div class="edit-box">
			<div class="label" style="width:100px;"><label>图片<font style="color:red;">(640*400)</font></label></div>
			<div class="input" id="singlepicdiv1">
				<img src='${news.pic}' width='100px' height='100px'/><input type='hidden' value='${news.pic}' name='pic'/>
			</div>
			<button type="button" class="btn btn-default" id="singlepic1"> 选择图片 </button>
		</div>
		<br/>
		<div class="edit-box">
		...
		<div class="edit-box">
			...
			<div class="">
				<select name="topflag">
					<option value="0" <c:if test="${news.topflag==0 }">selected="selected"</c:if>>否</option>
					<option value="1" <c:if test="${news.topflag==1 }">selected="selected"</c:if>>是</option>
				</select>
			</div>
		</div>
		<br/>
		<div class="edit-box">
			...
			<div class="">
				<input name="publishtime" id="day" type="text" onclick="SelectDate(this,'yyyy-MM-dd hh:mm')" readonly style="width:175px;cursor:pointer" value="<fmt:formatDate value='${news.publishtime}' pattern='yyyy-MM-dd HH:mm' />"/>
			</div>
		</div>
		<br/>
	</div>
