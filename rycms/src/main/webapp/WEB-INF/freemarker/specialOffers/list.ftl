...
<script language="javascript">
	...
	function badd() {
		window.location.href = "../preferentialInfo/add/";
	}
          ...
          <td width="96%"><table class="title-6" style="margin-left:8%;" width="80%"  >
              <tr>
                <td colspan="2"><form action="" method="">
                 ...
                 <#if pageModel??>
					<#list pageModel.results as info>                     
                      <tr align="center" class="d">
                        <td class="dd"><a href="../preferentialInfo/get?id=${info.id}"><font color="blue">${info.title}</font></a></td>
                        <td>${info.url}</td>
                         <td>${(info.createTime?string("yyyy-MM-dd HH:mm:ss"))!''}  </td>
                        <td class="dd"><a href="javascript:void(0)" onclick="confirmdelete(${info.id})"><font color="blue"> 删除 </font></a></td>                                                
                      </tr>
					</#list>
			     </#if>    
	      ...
	      <td colspan="7">
			    <div id="pager">
			  		...
			  		<#if pageModel.totalCount??>
			  			<@q.pager  pageNo=pageModel.currentPage  pageSize=pageModel.pageSize  recordCount=pageModel.totalCount  toURL="../preferentialInfo/list"  />
			  		</#if>
			    </div>	
		    </td>
       	</tr> 
