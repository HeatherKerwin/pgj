              ...
              <tr width="100%">
                ...
                <td width="90%" ><table width="100%">
                    <tr width="100%">
                      ...
                      <td width="10%">
                      <input style="width:120px;" type="text" class="orgId" id="orgId"/>
					  </td>
					  
					  <td width="8%" style="text-align:right;">手机号：</td>
                      <td width="5%">
                      <input style="width:120px;" type="text" class="phoneList" id="phoneList"/>
					  </td>
					  
					  <td width="8%" style="text-align:right;">日期：</td>
                      <td width="5%">
                      <input style="width:120px;" type="text" class="today" id="today"/>
					  </td>
					  ...
					  <td width="10%"><button type="button" onclick="xiugai()">修改</button></td>
					  <td ><button type="button" onclick="isprice()" >记录为恶意报价</button></td>
					  <td width="5%">&nbsp;</td>
					  <td ><button type="button"  onclick="isnotprice()" >非恶意报价</button></td>
                      <td width="7%">&nbsp;</td>
                    </tr>
                  </table></td>
                    <td width="5%" >&nbsp;</td>
...
<script type="text/javascript">
	...
	var orgId=${orgId};
	$.ajax({
			url: '${rc.contextPath}/historyprice/baojia/searchOrgAndName2',
			type: 'POST',
			dataType: 'json',
			data: {'id':orgId},
		...
		$("input").not(".orgId,.phoneList,.today, #demo").change(function(){
		if(/[^0-9.]/g.test(this.value)){alert("输入的不是数字");this.value='--';}
		});
	...
	var orgId=${orgId};
		$.ajax({
			url: '${rc.contextPath}/app/orgwarn/search/citylist',
			type: 'POST',
			dataType: 'json',
			data: {'orgId':orgId},
			success: function(result){
				var data = eval(result);
				var orgCityList = data.citys;
				if (orgCityList != null) {
					var str = '';
					$("#orgCityList").text('');
					for(var i in orgCityList){
						var orgCity = orgCityList[i];
						if (orgCity.city_id != null && orgCity.name != null) {
							str += "<option value='" + orgCity.city_id + "'>" + orgCity.name + "</option>";
						}
	 ...
	 //修改,根据页面上的类型分大小纸电分类保存
	 function xiugai(){
	 
	 var pt_id;
	 savebytype(1,1);savebytype(2,2);savebytype(21,21);
	 var tt = $("#tktType").val();
	 
	 if(tt == 0){
	 for(pt_id=3;pt_id <= 11;pt_id++){
	     savebytype(pt_id,pt_id);
	     }
	 }else if(tt == 1){
	 for(pt_id=12;pt_id <= 20;pt_id++){
	     savebytype(pt_id,pt_id);
	     }
	 }else{
	     alert("错了");
	 ...
	 function savebytype(num,t){
	  var pt_id=num; var priceTypeId=t;
	  var guogu;var dashang;var chengshang;var waizi;
	  var nongshang;var nonghe;var nongxin;var cunzhen;
	  var guogu2;var dashang2;var chengshang2;var waizi2;
	  var nongshang2;var nonghe2;var nongxin2;var cunzhen2;
	  
	  if( $(".pt_id" + pt_id + " .guogu").val() != "--" )
	  {guogu=$(".pt_id" + pt_id + " .guogu").val();}
	  if( $(".pt_id" + pt_id + " .guogu2").val() != "--")
	  {guogu2=$(".pt_id" + pt_id + " .guogu2").val();}
	  
	  if( $(".pt_id" + pt_id + " .dashang").val() != "--")
	  {dashang=$(".pt_id" + pt_id + " .dashang").val();}
	  if( $(".pt_id" + pt_id + " .dashang2").val() != "--")
	  {dashang2=$(".pt_id" + pt_id + " .dashang2").val();}
	  
	  if( $(".pt_id" + pt_id + " .chengshang").val() != "--")
	  {chengshang=$(".pt_id" + pt_id + " .chengshang").val();}
	  if( $(".pt_id" + pt_id + " .chengshang2").val() != "--")
	  {chengshang2=$(".pt_id" + pt_id + " .chengshang2").val();}
	  
	  if( $(".pt_id" + pt_id + " .waizi").val() != "--")
	  {waizi=$(".pt_id" + pt_id + " .waizi").val();}
	  if( $(".pt_id" + pt_id + " .waizi2").val() != "--")
	  {waizi2=$(".pt_id" + pt_id + " .waizi2").val();}
	  
	  if( $(".pt_id" + pt_id + " .nongshang").val() != "--")
	  {nongshang=$(".pt_id" + pt_id + " .nongshang").val();}
	  if( $(".pt_id" + pt_id + " .nongshang2").val() != "--")
	  {nongshang2=$(".pt_id" + pt_id + " .nongshang2").val();}
	  
	  if( $(".pt_id" + pt_id + " .nonghe").val() != "--")
	  {nonghe=$(".pt_id" + pt_id + " .nonghe").val();}
	  if( $(".pt_id" + pt_id + " .nonghe2").val() != "--")
	  {nonghe2=$(".pt_id" + pt_id + " .nonghe2").val();}
	  
	  if( $(".pt_id" + pt_id + " .nongxin").val() != "--")
	  {nongxin=$(".pt_id" + pt_id + " .nongxin").val();}
	  if( $(".pt_id" + pt_id + " .nongxin2").val() != "--")
	  {nongxin2=$(".pt_id" + pt_id + " .nongxin2").val();}
	  
	  if( $(".pt_id" + pt_id + " .cunzhen").val() != "--")
	  {cunzhen=$(".pt_id" + pt_id + " .cunzhen").val();}
	  if( $(".pt_id" + pt_id + " .cunzhen2").val() != "--")
	  {cunzhen2=$(".pt_id" + pt_id + " .cunzhen2").val();}
	  
	  var cityId = $("#orgCityList").val();
	  
	  var orgId=${orgId};
	  $.ajax({
	        url: '${rc.contextPath}/app/orgwarn/update',
			type: 'POST',
			data: {'cityId':cityId,'orgId':orgId ,'priceTypeId' : priceTypeId,'guogu':guogu,'guogu2':guogu2,'dashang':dashang,'dashang2':dashang2,'chengshang':chengshang,'chengshang2':chengshang2,'waizi':waizi,'waizi2':waizi2,'nongshang':nongshang,'nongshang2':nongshang2,'nonghe':nonghe,'nonghe2':nonghe2,'nongxin':nongxin,'nongxin2':nongxin2,'cunzhen':cunzhen,'cunzhen2':cunzhen2},
			dataType: 'json',
			success: function(result){
			var data = eval(result);
	 ...
	 //恶意报价
	function isprice(){
		var cf=confirm("你十分确定为恶意报价么！！！");
	if(cf == true){
	var id=${id};
	$.ajax({
			url: '${rc.contextPath}/app/orgwarn/eyi',
			type: 'POST',
			data: {'id':id},
			dataType: 'json',
			success: function(result){
			var data = eval(result);
				alert(data.state);
				window.document.location.href="${rc.contextPath}/app/orgwarn/list";
			}
		});
	}else if(cf == false){
	alert("下次小心点！")
	...
	//非恶意报价
	function isnotprice(){
    var cf=confirm("确定为非恶意报价么！！！");
	if(cf == true){
	var id=${id};
	$.ajax({
			url: '${rc.contextPath}/app/orgwarn/noteyi',
			type: 'POST',
			data: {'id':id},
			dataType: 'json',
			success: function(result){
	...
	function setType(){
		$.ajax({
			url: '${rc.contextPath}/historyprice/search/type',
			type: 'POST',
			data: {},
			dataType: 'json',
			success: function(result){
				var data = eval(result);
				var type1 = $("#tktType").val();
				if (type1 == 0) {	//纸票
	...
	//将所有页面中的输入框全部置为：--
	function setInput(){
		$("input").not(".orgId,.phoneList,.today, #demo").val("--");
		$(".orgId,.phoneList,.today, #demo").attr("readonly","readonly");//有些框不能输入
	}
    ...
    function showprice(num){
      ...
      var cityId = $("#orgCityList").val();
      $.ajax({
           url: '${rc.contextPath}/app/orgwarn/searchPrice',
           type: 'POST',
			dataType: 'json',
			data: {'orgId':orgId,'cityId':cityId},
			success: function(result) {
			var data = eval(result);
			showPriceData(data,num);
			}
		});
    }
    
    //将查询到的机构报价展示到页面
	function showPriceData(data,num){
