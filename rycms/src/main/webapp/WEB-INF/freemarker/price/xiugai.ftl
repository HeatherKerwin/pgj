              ...
              <tr width="100%">
                ...
                <td width="90%" ><table width="100%">
                    <tr width="100%">
                      ...
                      <td width="2%">&nbsp;</td>
                      <td width="15%"><input style="width: 100px;" class="btn" type="button" value="确认修改" onclick="xiugai()" /></td>
                      <td width="2%">&nbsp;</td>
                      <td width="15%"><input style="width: 100px;" class="btn" type="button" value="一键清空" onclick="clearData()" /></td>
                      <td width="1%">&nbsp;</td>
                    </tr>
                  </table></td>
                <td width="5%" >&nbsp;</td>
...
<script type="text/javascript">
	...
	function setCityList() {
		$.ajax({
			url: '../historyprice/getCityListByDay',
			type: 'POST',
			dataType: 'json',
			data: {},
			success: function(result){
				...
				if (cityList != null && cityList != "") {
					...
					for(var i in cityList){
						var city = cityList[i];
						if (city.cityid != null && city.name != null) {
					...
					var str = '';
					str += "<option value='107'>上海市</option>";
					$("#orgCityList").append(str);
				}
				clearData();
				loadData();
	...
	function setInputOnblur() {
		$(".guogu, .chengshang, .waizi, .nongshang, .nongxin, .nonghe, .cunzhen").bind("blur", function(){
			countPrice(this);
		});
		$(".guogu1, .chengshang1, .waizi1, .nongshang1, .nongxin1, .nonghe1, .cunzhen1").bind("blur", function(){
			countPrice1(this);
		});
		$(".guogu2, .chengshang2, .waizi2, .nongshang2, .nongxin2, .nonghe2, .cunzhen2").bind("blur", function(){
			countPrice2(this);
		});
	}
	...
	function loadData(){
		...
		$.ajax({
			url: '../historyprice/xiugai/searchHistoryPrice',
			type: 'POST',
			dataType: 'json',
			...
			success: function(result){
				...
				if (historyPriceList != null) {
					for(var i in historyPriceList){
						setHistoryPriceData(historyPriceList[i]);
					}
				}
			}
		});
	...
	//设置页面的数据(数据在后台组装)
	function setHistoryPriceData(data){
	...
	function xiugai() {
		...
		var cityId = $("#orgCityList").val();
		$.ajax({
			url: '../historyprice/xiugai/saveOrUpdatePrice',
			type: 'POST',
			dataType: 'json',
	...
	function showTypeElecData(data){
		...
		for(var i in typeList){
			...
			if(type1 == 0 && type2 == 1 && type5 == 1) { setTypeData("maiDuanElec", type, "(年利率)一年期"); }
	...
	function countPrice(obj){	// type_ 是用来取class的名称的（通过截取来获得）
		var className = $(obj).attr("class");		//该属性的class名称 -- 用于找到应往哪个地方赋值
	...
	function countPrice2(obj){
		var className = $(obj).attr("class");		//该属性的class名称 -- 用于找到应往哪个地方赋值
		className = className.substring(0, className.length-1);
		countType(obj, "B", className);
	}	
	
	//准备计算，开始获取需要的所有数据：obj-对象，style-A/B,className, money-金额, rate-利率
	function countType(obj, style, className) {
         var parentCname = $(obj).closest("table").attr("class");	//该属性所在的table中的class名称 --找地方
         var day = $(obj).closest("table").attr("day");				//该属性所在的table中的day的值	--计算时要用到的： 天数
         day = parseFloat(day);
         if (style == "A") {
         	//A方式要用户输入的利率和要加的参数，算出每10W多少钱
         	var rate = parseFloat($("." + parentCname + " ." + className).val());
         	if (isNaN(rate) || rate == "" || rate == undefined) {
	         	rate = 0;
	         }
         	var money = parseFloat($("." + parentCname + " ." + className + "1").val());
         	if (isNaN(money)) {
				money = 0;
			}
			countData(money, className, parentCname, day, style, rate);
         }else if(style == "B") {
	         var money = parseFloat($(obj).val());		//B方式要用户输入的每10W多少钱，算出利率
	         if (money == "" || money == undefined || isNaN(money)) {
	         	return;
	         }
	         countData(money, className, parentCname, day, style);
         }
    }
	
	//名称(money:A->B时代表利率，B->A代表金额)
	function countData(money, className, parentCname, day, style, rate) {
		console.log(day);
		if (style == "A") {			//方式A to 方式B
	        var countResult = (100000 * day * ((rate / 30) / 1000) + parseFloat(money)).toFixed(2);
	        $("." + parentCname + " ." + className + "2").val(countResult);
        }else if(style == "B"){		//方式B to 方式A
        	var countResult = (parseFloat((money * 30 * 1000) / 100000 / day)).toFixed(3);
	        $("." + parentCname + " ." + className).val(countResult);
	        $("." + parentCname + " ." + className + "1").val("0");
        }
    }
